...
class VQADataset(torch.utils.data.Dataset):
    def __init__(self, df_path, image_dir, transform=None, answer=True):
        ...
        # pdで読み込む
        class_mapping_df = pd.read_csv(io.StringIO(csv_content))
        class_mapping_df = class_mapping_df.rename(columns={'answer': 'answer', 'class_id': 'idx'})

        # マッピングを辞書に変換
        self.answer2idx = dict(zip(class_mapping_df['answer'], class_mapping_df['idx']))
    ...
    def __getitem__(self, idx):
        ...
        if self.answer:
            ...
            # print(answers)
            # print(mode_answer_idx)   
            # token =  [self.tokenizer([process_text(answer["answer"])],  return_tensors='pt', truncation=True, padding=True) for answer in self.df["answers"][idx]]
            # print(token)
            # token_ids = token['input_ids'][0].tolist()
            # print(token_ids)
...
def ResNet50_trained():
    ...
    model_ft.fc = nn.Linear(model_ft.fc.in_features, 1024)
...
class VQAModel(nn.Module):
    def __init__(self, vocab_size: int, n_answer: int):
        ...
        # self.image_transform = nn.Linear(512, 1024)

        # マルチヘッドAttention層
        # self.multihead_attention = nn.MultiheadAttention(embed_dim=768, num_heads=8)
        # self.multihead_attention1 = nn.MultiheadAttention(embed_dim=768, num_heads=8)
        # self.multihead_attention2 = nn.MultiheadAttention(embed_dim=1536, num_heads=8)
        ...
        # Transformerエンコーダ
        encoder_layer2 = nn.TransformerEncoderLayer(d_model=1024, nhead=8)
    ...
    def forward(self, image, question):
        ...
        image_feature2 = self.transformer_encoder(image_feature) 
        # image_feature = self.image_transform(image_feature)  # 次元を変換 (バッチサイズ, 768)
        question_feature = self.transformer_encoder2(question) 
        # print(question_feature.shape)

        # マルチヘッドAttention
        # atten_ima,weight = self.multihead_attention(image_feature,image_feature,image_feature)
        # atten_ima2,weight = self.multihead_attention1(atten_ima,atten_ima,atten_ima)
        ...
        combined_feature = torch.cat([image_feature2, question_feature], dim=1)  
        # Transformerエンコーダ
 # [batch_size, seq_len+1, embed_dim]
        
        # 最後のトークンの出力を使用して回答生成
        # print("Encoded Features Shape:", encoded_features.shape)
        output = self.fc(combined_feature)  # [batch_size, answer_size]
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),  # カラーのランダム調整
        # transforms.RandomHorizontalFlip(),  # ランダムに水平フリップ
